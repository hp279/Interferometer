...
public class TableFunction2 extends AbstractFunction2
{
  ...
  public TableFunction2(double minx, double maxx, double miny, double max, int m, int n)
  {
	z = new double[m][n];
 	init(minx, maxx, miny, maxy, z);
  }
  ...
  public double invoke(double x, double y)
  {
	...
	return ( z[i][j] * (partx-i+party-j) +
		     z[i+1][j] * (i+1-partx+party-j) + 
		     z[i][j+1] * (partx-i+j+1-party) + 
		     z[i+1][j+1] * (i+1-partx+j+1-party) ) / 2;
  }
