...
public abstract class AbstractFunction2 extends AbstractFunction  implements BinaryFunction<Double>
{
	FlatSet area;
 ...
 public Matrix22 diff2(double x, double y, double h)
  {
	double hh4 = 4*h*h;
	double a11 = (invoke(x+2*h, y) - 2*invoke(x, y) + invoke(x-2*h, y)) / hh4,
			a12 = (invoke(x+h, y+h) - invoke(x+h, y-h) - invoke(x-h, y+h) + invoke(x-h, y-h)) / hh4,
			a22 = (invoke(x, y+2*h) - 2*invoke(x, y) + invoke(x, y-2*h)) / hh4;
//	result.add( 0, 0, a11 );
//	result.add( 0, 1, a12 );
//	result.add( 1, 0, a12 );
//	result.add( 1, 1, a22 );
