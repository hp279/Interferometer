...
public class Utils
{
	  ...
	  static public Variable[] createVars(int mx, int my)
	  {
		  ...
		  for(int iy=0; iy<my; ++iy)// имена x1, x2, ... xm
			  vars[ix*my+iy] = Variable.make(String.format("x_%d,%d", ix+1, iy+1));
		  return vars;
	  }
	  ...
	  static public void initQuadraticFunction(Expression objex, double q[][], double c[])
	  {
		  int m = c.length;
		  for(int i=0; i<m; ++i)
			  for(int j=0; j<m; ++j)
