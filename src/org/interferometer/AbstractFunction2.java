...
public abstract class AbstractFunction2 implements BinaryFunction<Double>
{
 ...
 public double[] diff(double x, double y, double h)
  ...
  public double[][] diff2(double x, double y, double h)
  {
	...
	double hh4 = 4*h*h;
	result[0][0] = (invoke(x+2*h, y) - 2*invoke(x, y) + invoke(x-2*h, y)) / hh4;
	result[0][1] = result[1][0] = (invoke(x+h, y+h) - invoke(x+h, y-h) - invoke(x-h, y+h) + invoke(x-h, y-h)) / hh4;
	result[1][1] = (invoke(x, y+h) - 2*invoke(x, y) + invoke(x, y-h)) / hh4;
	return result;
  }
}