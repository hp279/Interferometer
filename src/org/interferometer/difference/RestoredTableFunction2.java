...
import org.interferometer.linear.Utils;
...
{ 
  ...
  double h; // характерный интервал, на котором имеет смысл оценивать модуль непрерывности
...
//		  for(int i=0; i<m_x; ++i)
//			  for(int j=0; j<m_y; ++j)
//				  System.out.printf(" (%d, %d)", base_x[i][j], base_y[i][j]);
	  }
	  ...
	  // массив разностей, позволяющих восстановить функцию во всей области по значениям в квадрате 
	  // TODO: сейчас возможно только вычисление по возрастанию x и y - то есть только базовые северо-западные номера
	  // сделать для любых базовых номеров
	  public double[][] getDifferences(Difference2Info diff)
	  {
		  int nx = getSizeX() + 1,
			  ny = getSizeY() + 1; // количество значений	  
		  double diffs[][] = new double[nx][ny];
		  for(int ix=0; ix<m_x; ++ix)
		  for(int iy=0; iy<m_y; ++iy)
		  {
			  int xbase = base_x[ix][iy],
				  ybase = base_y[ix][iy];
			  for(int jx=ix; jx<nx; jx+=m_x)
			  {
				  double x = getArgument1(jx) + (jx==0? 1e-10 : (jx==nx-1? -1e-10 : 0)); // чтобы всё попало в интервалы
				  // движемся при данном x по y от ybase сперва в уменьшение, потом в увеличение
				  ...
				  for(int jy=ybase+m_y;;)
				  {
					  ...
					  double y = getArgument2(jy) + (jy==0? 1e-10 : (jy==ny-1? -1e-10 : 0)); // чтобы всё попало в интервалы	
					  if(!hasArgument(x, y))
						  continue;
					  if(jx == xbase)
					  if(jy == ybase)
						  ...
						  diffs[jx][jy] = diffs[jx][jy-m_y] + diff.getDeltaY().invoke(x, y);
					  else if(jy == ybase)
					  	  diffs[jx][jy] = diffs[jx-m_x][jy] + diff.getDeltaX().invoke(x, y);
					  else
					  {	 
						  if(hasArgument(x - diff.getS(), y))
						  {
							  if(back)
								  if(hasArgument(x, y + diff.getS()))
									  // чтобы минимизировать погрешности, берем среднее арифметическое
									  diffs[jx][jy] = (diffs[jx-m_x][jy] + diff.getDeltaX().invoke(x, y) +
											  			diffs[jx][jy+m_y] - diff.getDeltaY().invoke(x, y + diff.getS())) / 2;
								  else
									  diffs[jx][jy] = diffs[jx-m_x][jy] + diff.getDeltaX().invoke(x, y);							  
							  else
								  if(hasArgument(x, y - diff.getS()))
									  // чтобы минимизировать погрешности, берем среднее арифметическое
									  diffs[jx][jy] = (diffs[jx-m_x][jy] + diff.getDeltaX().invoke(x, y) +
											  		   diffs[jx][jy-m_y] + diff.getDeltaY().invoke(x, y)) / 2;
								  else
									  diffs[jx][jy] = diffs[jx-m_x][jy] + diff.getDeltaX().invoke(x, y);
						  }
						  else
							  if(back)
								  diffs[jx][jy] = diffs[jx][jy+m_y] - diff.getDeltaY().invoke(x, y + diff.getS());
							  else
								  diffs[jx][jy] = diffs[jx][jy-m_y] + diff.getDeltaY().invoke(x, y);
					  }
			  	}
	  ...
	  void addDiffByXSquares(Difference2Info diff_info, double diffs[][], double q[][], double c[])
	  {
		  ...
		  int nx = getSizeX() + 1,
 		  ...
 		  {
 			  double x = getArgument1(ix),
 					 y = getArgument2(iy);
 			  if(hasArgument(x, y) && hasArgument(x-diff_info.getS(), y))
 			  {
 				  int jx = ix % m_x,
 						  jy = iy % m_y;
 				  int jsix = (ix-six) % m_x; 
 				  q [jx*m_y+jy] [jx*m_y+jy] ++;
 				  q[jsix*m_y+jy] [jsix*m_y+jy] ++;
 				  q[jx*m_y+jy] [jsix*m_y+jy] --;
 				  q[jsix*m_y+jy] [jx*m_y+jy] --;
 			  
 				  c[jx*m_y+jy] += 2*diffs[ix][iy];
 				  c[jx*m_y+jy] -= 2*diff_info.getDeltaX(x, y);
 				  c[jx*m_y+jy] -= 2*diffs[ix-six][iy];
 				  c[jsix*m_y+jy] += 2*diffs[ix-six][iy];
 				  c[jsix*m_y+jy] += 2*diff_info.getDeltaX(x, y);
 				  c[jsix*m_y+jy] -= 2*diffs[ix][iy];
 			  }
 		  }
	  }
	  
	  void addDiffByYSquares(Difference2Info diff_info, double diffs[][], double q[][], double c[])
	  {
 		  // TODO: что делать, если s не делится на число пикселей?
		  int nx = getSizeX() + 1,
			  ny = getSizeY() + 1; // количество значений	  
 		  ...
 		  for(int iy=siy; iy<ny; iy++)
 		  {
 			  double x = getArgument1(ix),
 					 y = getArgument2(iy);
 			  if(hasArgument(x, y) && hasArgument(x, y-diff_info.getS()))
 			  {
 				  int jx = ix % m_x,
 					  jy = iy % m_y;
 				  int jsiy = (iy-siy) % m_y;
 				  ...
 				  q [jx*m_y+jy] [jx*m_y+jy] ++;
 	 			  q [jx*m_y+jsiy] [jx*m_y+jsiy] ++;
 	 			  q [jx*m_y+jy] [jx*m_y+jsiy] --;
 	 			  q [jx*m_y+jsiy] [jx*m_y+jy] --;
 	 			  
 	 			  c[jx*m_y+jy] += 2*diffs[ix][iy];
 	 			  c[jx*m_y+jy] -= 2*diff_info.getDeltaY(x, y);
 	 			  c[jx*m_y+jy] -= 2*diffs[ix][iy-siy];
 	 			  c[jx*m_y+jsiy] += 2*diffs[ix][iy-siy];
 	 			  c[jx*m_y+jsiy] += 2*diff_info.getDeltaY(x, y);
 	 			  c[jx*m_y+jsiy] -= 2*diffs[ix][iy];
  			  }
 		  }
	  }
  }
  
  public RestoredTableFunction2(double minx, double maxx, double miny, double maxy, int m, int n, 
		  AbstractFunction1 cont_modul, double h)
  {
  ...
  private void setRestrictions(ExpressionsBasedModel<?> model, double s, double[][] diffs)
  {
	  int mx = (int)(s/getStepX() + 1e-10),
		  my = (int)(s/getStepY() + 1e-10),
		  m = mx * my; // количество переменных
	  int nx = getSizeX() + 1,
		  ny = getSizeY() + 1; // общее количество значений
	  int dx = (int)(this.h/getStepX()),
		  dy = (int)(this.h/getStepY());
	  ...
	  for(int j2x = Math.max(j1x-dx, 0); j2x < Math.min(j1x+dx, mx); ++j2x)
	  for(int j2y = Math.max(j1y-dy, 0); j2y < Math.min(j1y+dy, my); ++j2y)
	  {
			  int j1 = j1x * my + j1y,
				  j2 = j2x * my + j2y;
			  ...
			  mindiffs[j1][j2] = 1e300; // как по-человечески сделать бесконечность??????????????????
			  for(int i1x=j1x; i1x<nx; i1x+=mx)
			  for(int i1y=j1y; i1y<ny; i1y+=my)
			  for(int i2x=Math.max(i1x-dx, 0); i2x<Math.min(i1x+dx, nx); i2x+=mx)
			  for(int i2y=Math.max(i1y-dy, 0); i2y<Math.min(i1y+dy, ny); i2y+=my)  
			  {
				  double maxdiff = cont_modul.invoke(getDistance(i1x, i1y, i2x, i2y));   
				  mindiffs[j1][j2] = Math.min(mindiffs[j1][j2], maxdiff-(diffs[i2x][i2y] - diffs[i1x][i1y]));
			  }
			  Expression ex = model.addEmptyLinearExpression(String.format("((%d, %d), (%d, %d))", j1x, j1y, j2x, j2y));
			  for(int j=0; j<m; ++j)
			  {
  ...
  private void restoreSolverResult(BaseNumbers numbers, Result result, double[][] diffs)
  {
	  int nx = getSizeX() + 1,
			  ny = getSizeY() + 1;
	  ...
	  for(int ix=0; ix<nx; ++ix)
	  for(int iy=0; iy<ny; ++iy)
			this.setValue(ix, iy, vector.toBigDecimal(numbers.getBaseVariable(ix, iy), 0).doubleValue() + diffs[ix][iy]);
	  this.setDefined();
  }
 ...
 // восстановление функции по сдвигу по с заданным шагом и величиной сдвига
  public void restoreByShift(Difference2Info diff)
  {
	  ...
	  Variable vars[] = Utils.createVars(mx, my);
  ...
  public void restoreByShift(Difference2Info[] diffs)
  {
	  ...
	  Expression objex = model.addEmptyCompoundExpression("y");
	  objex.setContributionWeight(new BigDecimal(1)); // 1 - минимизация
	  double[][] q = new double[m][m]; // Квадратичная часть целевой функции.
	  double[] c = new double[m]; 	  // Линейная часть целевой функции:
	  // Определяем линейную и квадратичную часть:
	  Utils.makeZeroQuadraticFunction(q, c);
 	  for(int i = 1; i<diffs.length; ++i)
 	  {
 		  numbers.addDiffByXSquares(diffs[i], diff_values, q, c);
 		  numbers.addDiffByYSquares(diffs[i], diff_values, q, c);
  	  }

    // Инициализируем целевую функцию:
 	Utils.initQuadraticFunction(objex, q, c);
	// Решаем задачу:
	System.out.println("begin solving...");
	OptimisationSolver solver = model.getDefaultSolver();
	//System.out.printf("Matrices: \n%s", solver.toString());
