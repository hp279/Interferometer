...
{ 
  ...
  public RestoredTableFunction(double min, double max, int n, AbstractFunction cont_modul)
  {
	super(min, max, n);
 ...
 // восстановление функции по сдвигу с заданным шагом и величиной сдвига
  public void restoreByShift(AbstractFunction deltaf, double s)
  {
	  ...
	  // на которое делится y.length!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  int m = (int)(s/delta + 1e-10); // количество переменных
	  int n = y.length; // общее количество значений
	  int K = n / m; // количество разностей, укладывающихся в области определения
	  ...
	  for(int i=m; i<n; ++i)
	  {
		  double xi = this.getArgument(i) + (i==0? 1e-10 : (i==n-1? -1e-10 : 0)); // чтобы всё попало в интервалы
  ...
  // восстановление функции по двум сдвигам с заданным шагом и величинами сдвигов
  public void restoreByShift(AbstractFunction delta1f, double s1, AbstractFunction delta2f, double s2)
  {
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  }
